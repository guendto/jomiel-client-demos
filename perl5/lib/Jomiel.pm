...
sub new {
  ...
  $gpd->map(
    ...
    {
      message => $pkg_path_dot . '.MediaInquiry',
      to      => 'MediaInquiry'
    },
    {
      message => $pkg_path_dot . '.MediaResponse',
      to      => 'MediaResponse'
    },
    {
      message => $pkg_path_dot . '.ResponseStatus',
      to      => 'ResponseStatus'
    },
...
sub dump_terse_response {
  ...
  for (my $i = 0; $i < $media_response->stream_size; $i++) {
    ...
    say sprintf("  profile: %s\n    width: %d\n    height: %d",
      $quality->get_profile, $quality->get_width, $quality->get_height);
  }
}
